...
import { BENTLEY_HUB, bentleyStockHref } from "@/lib/make-hubs/bentley";
import {
  STOCK_PAGE_SIZE,
  buildStockHref,
  filtersFromStockSp,
...
export async function generateMetadata({
  ...
  if (firstStockParam(sp.model)) {
    return { title: BENTLEY_HUB.title, robots: { index: false, follow: true } };
  }
  ...
  return {
    title: page > 1 ? `${BENTLEY_HUB.title} | Page ${page}` : BENTLEY_HUB.title,
    description: BENTLEY_HUB.description,
    alternates: { canonical },
    openGraph: {
      title: BENTLEY_HUB.title,
      description: BENTLEY_HUB.description,
...
export default async function BentleyStockHubPage({
  searchParams,
}: {
  searchParams: Promise<SP>;
}) {
  ...
  return (
    <main className="stock-page make-hub-page">
      ...
      <JsonLd data={faqJsonLd([...BENTLEY_HUB.faqs])} />
      ...
      <div className="container make-hub-shell">
        <header className="stock-hero make-hub-hero">
          <div className="stock-meta">
            ...
            <span>
              {total > 0
                ? `${total} Bentley${total === 1 ? "" : "s"} in stock`
                : "Sourcing Bentleys in Japan"}
            </span>
          </div>
          <h1 className="stock-title">{BENTLEY_HUB.h1}</h1>
          <p className="stock-lead make-hub-intro">{BENTLEY_HUB.intro}</p>
        </header>

        <section className="make-hub-stock" aria-label="Bentley stock">
          ...
          {items.length > 0 ? (
            <>
              <StockViewItemListTracker vehicles={items} listName="bentley_hub" />
              <div className="vehicle-grid stock-grid">
                {items.map((v) => (
                  <VehicleCard key={v.id} v={v} listName="bentley_hub" />
                ))}
              </div>
            ...
            <div className="stock-empty glass make-hub-empty">
              <h3>No Bentleys listed right now</h3>
              <p className="muted">
                We search Japanese dealer stock and auctions for the Continental GT, Flying Spur,
                Mulsanne, Bentayga, Arnage and more. Tell us what you want.
              </p>
              <div className="stock-source-actions">
                <Link className="btn btn-gold" href="/about#contact-form">
                  Request a Bentley
                </Link>
                <WhatsAppLink className="btn btn-outline" location="bentley_hub_empty">
                  WhatsApp us
                </WhatsAppLink>
              </div>
            </div>
        ...
        <aside className="stock-source-cta glass make-hub-cta">
          <h2 className="heading">{BENTLEY_HUB.closing.title}</h2>
          <p className="muted">{BENTLEY_HUB.closing.body}</p>
          <div className="stock-source-actions">
            <Link className="btn btn-gold" href="/about#contact-form">
              Contact us
            </Link>
            <WhatsAppLink className="btn btn-outline" location="bentley_hub_cta">
              WhatsApp us
            </WhatsAppLink>
          </div>
        </aside>
        ...
        <article className="make-hub-article">
          {BENTLEY_HUB.lead.slice(1).map((p) => (
          ...
          <section className="make-hub-block" aria-labelledby="short-answer">
            <h2 id="short-answer" className="heading">
              {BENTLEY_HUB.shortAnswer.title}
            </h2>
            {BENTLEY_HUB.shortAnswer.paragraphs.map((p) => (
              <p key={p.slice(0, 40)} className="make-hub-prose">
                {p}
              </p>
            ))}
          ...
          <section className="make-hub-block" aria-labelledby="comparison">
            <h2 id="comparison" className="heading">
              Ten Bentleys worth searching for
            </h2>
            <div className="make-hub-table-wrap">
              <table className="make-hub-table">
                ...
                <tbody>
                  {BENTLEY_HUB.comparison.map((row) => (
                    <tr key={row.model}>
                      <td>{row.want}</td>
                      <td>
          ...
          <section className="make-hub-block" aria-labelledby="how-to-choose">
            <h2 id="how-to-choose" className="heading">
              {BENTLEY_HUB.howToChoose.title}
            </h2>
            {BENTLEY_HUB.howToChoose.paragraphs.map((p) => (
              <p key={p.slice(0, 40)} className="make-hub-prose">
                {p}
              </p>
            ))}
        ...
        <section className="make-hub-faq" aria-labelledby="bentley-faq">
          <h2 id="bentley-faq" className="heading">
            Frequently asked questions
          </h2>
          <Faq items={[...BENTLEY_HUB.faqs]} />
        </section>
      </div>
    </main>
  );
