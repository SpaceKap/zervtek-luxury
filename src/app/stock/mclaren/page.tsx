...
import { MCLAREN_HUB, mclarenStockHref } from "@/lib/make-hubs/mclaren";
import {
  STOCK_PAGE_SIZE,
  buildStockHref,
  filtersFromStockSp,
...
export async function generateMetadata({
  ...
  if (firstStockParam(sp.model)) {
    return { title: MCLAREN_HUB.title, robots: { index: false, follow: true } };
  }
  ...
  return {
    title: page > 1 ? `${MCLAREN_HUB.title} | Page ${page}` : MCLAREN_HUB.title,
    description: MCLAREN_HUB.description,
    alternates: { canonical },
    openGraph: {
      title: MCLAREN_HUB.title,
      description: MCLAREN_HUB.description,
...
export default async function McLarenStockHubPage({
  searchParams,
}: {
  searchParams: Promise<SP>;
}) {
  ...
  return (
    <main className="stock-page make-hub-page">
      ...
      <JsonLd data={faqJsonLd([...MCLAREN_HUB.faqs])} />
      ...
      <div className="container make-hub-shell">
        <header className="stock-hero make-hub-hero">
          <div className="stock-meta">
            ...
            <span>
              {total > 0
                ? `${total} McLaren${total === 1 ? "" : "s"} in stock`
                : "Sourcing McLarens in Japan"}
            </span>
          </div>
          <h1 className="stock-title">{MCLAREN_HUB.h1}</h1>
          <p className="stock-lead make-hub-intro">{MCLAREN_HUB.intro}</p>
        </header>

        <section className="make-hub-stock" aria-label="McLaren stock">
          ...
          {items.length > 0 ? (
            <>
              <StockViewItemListTracker vehicles={items} listName="mclaren_hub" />
              <div className="vehicle-grid stock-grid">
                {items.map((v) => (
                  <VehicleCard key={v.id} v={v} listName="mclaren_hub" />
                ))}
              </div>
            ...
            <div className="stock-empty glass make-hub-empty">
              <h3>No McLarens listed right now</h3>
              <p className="muted">
                We search Japanese dealer stock and auctions for the 12C, 570S, 720S, 765LT,
                Artura, 750S and more. Tell us what you want.
              </p>
              <div className="stock-source-actions">
                <Link className="btn btn-gold" href="/about#contact-form">
                  Request a McLaren
                </Link>
                <WhatsAppLink className="btn btn-outline" location="mclaren_hub_empty">
                  WhatsApp us
                </WhatsAppLink>
              </div>
            </div>
        ...
        <aside className="stock-source-cta glass make-hub-cta">
          <h2 className="heading">{MCLAREN_HUB.closing.title}</h2>
          <p className="muted">{MCLAREN_HUB.closing.body}</p>
          <div className="stock-source-actions">
            <Link className="btn btn-gold" href="/about#contact-form">
              Contact us
            </Link>
            <WhatsAppLink className="btn btn-outline" location="mclaren_hub_cta">
              WhatsApp us
            </WhatsAppLink>
          </div>
        </aside>
        ...
        <article className="make-hub-article">
          {MCLAREN_HUB.lead.slice(1).map((p) => (
          ...
          <section className="make-hub-block" aria-labelledby="short-answer">
            <h2 id="short-answer" className="heading">
              {MCLAREN_HUB.shortAnswer.title}
            </h2>
            {MCLAREN_HUB.shortAnswer.paragraphs.map((p) => (
              <p key={p.slice(0, 40)} className="make-hub-prose">
                {p}
              </p>
            ))}
          ...
          <section className="make-hub-block" aria-labelledby="comparison">
            <h2 id="comparison" className="heading">
              Ten McLarens worth searching for
            </h2>
            <div className="make-hub-table-wrap">
              <table className="make-hub-table">
                ...
                <tbody>
                  {MCLAREN_HUB.comparison.map((row) => (
                    <tr key={row.model}>
                      <td>{row.want}</td>
                      <td>
          ...
          <section className="make-hub-block" aria-labelledby="how-to-choose">
            <h2 id="how-to-choose" className="heading">
              {MCLAREN_HUB.howToChoose.title}
            </h2>
            {MCLAREN_HUB.howToChoose.paragraphs.map((p) => (
              <p key={p.slice(0, 40)} className="make-hub-prose">
                {p}
              </p>
            ))}
        ...
        <section className="make-hub-faq" aria-labelledby="mclaren-faq">
          <h2 id="mclaren-faq" className="heading">
            Frequently asked questions
          </h2>
          <Faq items={[...MCLAREN_HUB.faqs]} />
        </section>
      </div>
    </main>
  );
