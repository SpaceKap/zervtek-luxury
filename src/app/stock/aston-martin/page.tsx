...
import { ASTON_MARTIN_HUB, astonMartinStockHref } from "@/lib/make-hubs/aston-martin";
import {
  STOCK_PAGE_SIZE,
  buildStockHref,
  filtersFromStockSp,
...
const HUB_PATH = "/stock/aston-martin";
...
export async function generateMetadata({
  ...
  if (firstStockParam(sp.model)) {
    return { title: ASTON_MARTIN_HUB.title, robots: { index: false, follow: true } };
  }
  ...
  return {
    title: page > 1 ? `${ASTON_MARTIN_HUB.title} | Page ${page}` : ASTON_MARTIN_HUB.title,
    description: ASTON_MARTIN_HUB.description,
    alternates: { canonical },
    openGraph: {
      title: ASTON_MARTIN_HUB.title,
      description: ASTON_MARTIN_HUB.description,
...
export default async function AstonMartinStockHubPage({
  searchParams,
}: {
  searchParams: Promise<SP>;
}) {
  ...
  return (
    <main className="stock-page make-hub-page">
      ...
      <JsonLd data={faqJsonLd([...ASTON_MARTIN_HUB.faqs])} />
      ...
      <div className="container make-hub-shell">
        <header className="stock-hero make-hub-hero">
          <div className="stock-meta">
            ...
            <span>
              {total > 0
                ? `${total} Aston Martin${total === 1 ? "" : "s"} in stock`
                : "Sourcing Aston Martins in Japan"}
            </span>
          </div>
          <h1 className="stock-title">{ASTON_MARTIN_HUB.h1}</h1>
          <p className="stock-lead make-hub-intro">{ASTON_MARTIN_HUB.intro}</p>
        </header>

        <section className="make-hub-stock" aria-label="Aston Martin stock">
          ...
          {items.length > 0 ? (
            <>
              <StockViewItemListTracker vehicles={items} listName="aston_martin_hub" />
              <div className="vehicle-grid stock-grid">
                {items.map((v) => (
                  <VehicleCard key={v.id} v={v} listName="aston_martin_hub" />
                ))}
              </div>
            ...
            <div className="stock-empty glass make-hub-empty">
              <h3>No Aston Martins listed right now</h3>
              <p className="muted">
              ...
              <div className="stock-source-actions">
                ...
                <WhatsAppLink className="btn btn-outline" location="aston_martin_hub_empty">
                  WhatsApp us
                </WhatsAppLink>
              </div>
            </div>
        ...
        <aside className="stock-source-cta glass make-hub-cta">
          <h2 className="heading">{ASTON_MARTIN_HUB.closing.title}</h2>
          <p className="muted">{ASTON_MARTIN_HUB.closing.body}</p>
          <div className="stock-source-actions">
            <Link className="btn btn-gold" href="/about#contact-form">
              Contact us
            </Link>
            <WhatsAppLink className="btn btn-outline" location="aston_martin_hub_cta">
              WhatsApp us
            </WhatsAppLink>
          </div>
        </aside>
        ...
        <article className="make-hub-article">
          {ASTON_MARTIN_HUB.lead.slice(1).map((p) => (
          ...
          <section className="make-hub-block" aria-labelledby="short-answer">
            <h2 id="short-answer" className="heading">
              {ASTON_MARTIN_HUB.shortAnswer.title}
            </h2>
            {ASTON_MARTIN_HUB.shortAnswer.paragraphs.map((p) => (
              <p key={p.slice(0, 40)} className="make-hub-prose">
                {p}
              </p>
            ))}
          ...
          <section className="make-hub-block" aria-labelledby="comparison">
            <h2 id="comparison" className="heading">
              Ten Aston Martins worth searching for
            </h2>
            <div className="make-hub-table-wrap">
              <table className="make-hub-table">
                ...
                <tbody>
                  {ASTON_MARTIN_HUB.comparison.map((row) => (
                    <tr key={row.model}>
                      <td>{row.want}</td>
                      <td>
          ...
          <section className="make-hub-block" aria-labelledby="how-to-choose">
            <h2 id="how-to-choose" className="heading">
              {ASTON_MARTIN_HUB.howToChoose.title}
            </h2>
            {ASTON_MARTIN_HUB.howToChoose.paragraphs.map((p) => (
              <p key={p.slice(0, 40)} className="make-hub-prose">
                {p}
              </p>
            ))}
        ...
        <section className="make-hub-faq" aria-labelledby="aston-martin-faq">
          <h2 id="aston-martin-faq" className="heading">
            Frequently asked questions
          </h2>
          <Faq items={[...ASTON_MARTIN_HUB.faqs]} />
        </section>
      </div>
    </main>
  );
